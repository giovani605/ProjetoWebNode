...
function buscarReservasPorRestaurante(idrestaurante, callback) {
  var query =
    "select r.idreserva, r.data_reserva, r.idpromocao , " +
    " p.nome  nomePrato, p.idimagem, p.descricao, res.idrestaurante, " +
    " res.nome  nomeRestaurante, promo.codigo " +
    " from reservas r inner join  pratos p on r.idprato = p.idpratos inner join  restaurante  res " +
    " on p.restaurante_idrestaurante = res.idrestaurante inner join promocao promo on promo.idprato = p.idpratos " +
    " where res.idrestaurante = " +
    idrestaurante +
    " ;";
  console.log(query);
...
function buscarReservasPorUsuario(idUser, callback) {
  ...
  /*var query =
    "select reservas.idreserva, reservas.data_reserva, reservas.idpromocao, pratos.nome as nomePrato, pratos.idimagem, pratos.descricao, restaurante.idrestaurante, restaurante.nome as nomeRestaurante " +
...
function validaPromocao(idPrato, codigo, callback) {
  var query =
    "select promocao.promocaoid from promocao where codigo = '" +
    codigo +
    "' and promocao.idprato = " +
    idPrato +
    " limit 1;";
  console.log(query);
  conexao.query(query, (err, res) => {
...
function insertReservaSQL(idPrato, idUsuario, data, idPromocao, callback) {
  var query =
    "insert into reservas (idprato, idusuario, data_reserva, idpromocao) values (" +
    idPrato +
    ", " +
    idUsuario +
    ", '" +
